...
const Hero:React.FC=()=>{
 const {stats,loading,error:statsError}=useGitHubStats();
 const [downloadLoading,setDownloadLoading]=useState(false),[error,setError]=useState(''),[role,setRole]=useState(0),[avatarFailed,setAvatarFailed]=useState(false);
 ...
 <div className="role-rotator" aria-live="polite"><span>I work across:</span><strong>{roles[role]}</strong></div><div className="hero-actions"><a href="#ai-chat" className="btn btn-primary">Ask My AI Assistant →</a><button className="btn btn-tertiary" onClick={download} disabled={downloadLoading} aria-busy={downloadLoading}>{downloadLoading?'Preparing download…':'Download Resume'}</button></div>
 {error&&<p role="status">{error}</p>}{statsError&&<p className="github-secondary" role="status">Live GitHub statistics are unavailable. Showing the local profile backup.</p>}{loading?<p>Loading GitHub statistics…</p>:stats?<div className="github-secondary" aria-label="GitHub statistics"><strong>{stats.profile.public_repositories}</strong> repositories · <strong>{stats.repository_stats.total_stars}</strong> stars · <strong>{stats.repository_stats.total_forks}</strong> forks · <strong>{stats.profile.followers}</strong> followers · <strong>{stats.contributions.total}</strong> contributions<br/>Top languages: {stats.top_languages.map(x=>`${x.name} ${x.percentage}%`).join(' · ')||'Not available'}</div>:null}
 </div><div className="hero-profile">{stats?.profile.avatar_url&&!avatarFailed?<img src={stats.profile.avatar_url} alt={stats.profile.name||stats.username} className="profile-image" onError={()=>setAvatarFailed(true)}/>:<div className="profile-image profile-image-fallback" role="img" aria-label={`${fallbackProfile.name} profile placeholder`}>{fallbackProfile.initials}</div>}<div className="profile-card-copy"><span>Backend • AI • Automation</span><p>Building systems that connect frontend UX, APIs, data, AI workflows, and deployment.</p></div></div></div></div></section>
};export default Hero;
