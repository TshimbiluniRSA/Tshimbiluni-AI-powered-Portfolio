...
class GitHubProfileBase(BaseModel):
    ...
    username: str = Field(
        ...
        min_length=1,
        max_length=39,
        description="GitHub username (max 39 characters)",
        example="TshimbiluniRSA",
    )
    bio: Optional[str] = Field(None, max_length=1000, description="User's GitHub bio")
    public_repos: Optional[int] = Field(
        None, ge=0, description="Number of public repositories"
    )
    followers: Optional[int] = Field(None, ge=0, description="Number of followers")
    following: Optional[int] = Field(
        None, ge=0, description="Number of users being followed"
    )
    profile_url: Optional[HttpUrl] = Field(
        None, description="Full URL to GitHub profile"
    )
    avatar_url: Optional[HttpUrl] = Field(
        None, description="URL to profile avatar image"
    )
    name: Optional[str] = Field(
        None, max_length=255, description="Display name on GitHub"
    )
    company: Optional[str] = Field(
        None, max_length=255, description="Company information"
    )
    location: Optional[str] = Field(
        None, max_length=255, description="Location information"
    )
    blog: Optional[HttpUrl] = Field(None, description="Blog/website URL")
    twitter_username: Optional[str] = Field(
        None, max_length=15, description="Twitter username"
    )
...
class GitHubProfileResponse(GitHubProfileBase, BaseResponseModel):
    ...
    last_fetched_at: Optional[datetime] = Field(
        None, description="When the data was last fetched from GitHub API"
    )
...
class CVMetadataBase(BaseModel):
    filename: str = Field(
        ...
        example="Tshimbiluni_Nedambale_CV.pdf",
    )
    filepath: str = Field(
        ..., min_length=1, max_length=500, description="Storage path of the CV file"
    )
    file_size: Optional[int] = Field(
        None, ge=0, le=10 * 1024 * 1024, description="File size in bytes"
    )
    file_type: Optional[str] = Field(
        None, max_length=50, description="MIME type of the file"
    )
    version: int = Field(1, ge=1, description="Version number of the CV")
    is_active: bool = Field(
        True, description="Whether this CV version is currently active"
    )
    description: Optional[str] = Field(
        None, description="Description or notes about this CV version"
    )
...
class CVMetadataResponse(CVMetadataBase, BaseResponseModel):
    ...
    download_count: int = Field(
        0, ge=0, description="Number of times this CV has been downloaded"
    )
...
class ChatMessageBase(BaseModel):
    session_id: str = Field(
        ..., min_length=1, max_length=255, description="Chat session identifier"
    )
    message_type: MessageType = Field(
        ..., description="Type of message: 'user', 'assistant', or 'system'"
    )
    content: str = Field(..., min_length=1, description="Message content")
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional metadata like tokens used, model info, etc."
    )
...
class ChatMessageResponse(ChatMessageBase, BaseResponseModel):
    ...
    response_time_ms: Optional[int] = Field(
        None, ge=0, description="Response time in milliseconds for assistant messages"
    )
    tokens_used: Optional[int] = Field(
        None, ge=0, description="Number of tokens used for this message"
    )
    model_used: Optional[str] = Field(
        None, max_length=100, description="AI model used for generating the response"
    )
    rating: Optional[int] = Field(
        None, ge=1, le=5, description="User rating for the response (1-5)"
    )
...
class ChatSessionResponse(BaseResponseModel):
    ...
    message_count: int = Field(
        ge=0, description="Total number of messages in the session"
    )
...
class APIUsageLogBase(BaseModel):
    ...
    api_provider: APIProvider = Field(
        ..., description="API provider (github or openai)"
    )
    ...
    tokens_used: Optional[int] = Field(
        None, ge=0, description="Tokens used (for LLM APIs)"
    )
    cost_usd: Optional[float] = Field(
        None, ge=0, description="Cost in USD (if applicable)"
    )
...
class HealthCheckResponse(BaseModel):
    ...
    external_apis: Dict[str, bool] = Field(
        default_factory=dict, description="External API connectivity status"
    )
...
class SyncResponse(BaseModel):
    ...
    errors: Optional[List[str]] = Field(
        None, description="List of errors that occurred during sync"
    )
...
class GitHubSyncRequest(BaseModel):
    ...
    username: str = Field(
        ..., min_length=1, max_length=39, description="GitHub username to sync"
    )
    force_refresh: bool = Field(
        False, description="Force refresh even if data is not stale"
    )
...
class CVUploadResponse(BaseModel):
    ...
    cv_metadata: Optional[CVMetadataResponse] = Field(
        None, description="CV metadata if upload was successful"
    )
    file_url: Optional[str] = Field(None, description="URL to access the uploaded file")


class GitHubStatsProfile(BaseModel):
...
class GitHubRepositoryStats(BaseModel):
    ...
    total_repository_size_kb: int = 0
...
class GitHubRecentRepository(BaseModel):
    name: str
    description: Optional[str] = None
...
class GitHubStatsResponse(BaseModel):
    username: str
    profile: GitHubStatsProfile
    repository_stats: GitHubRepositoryStats
