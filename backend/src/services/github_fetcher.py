...
class GitHubService:
        ...
        try:
            async with httpx.AsyncClient(timeout=DEFAULT_TIMEOUT) as client:
                response = await client.request(
                    method=method, url=url, headers=self.headers, params=params or {}
                )
    ...
    async def fetch_user_profile(
        ...
        profile_data = await self._make_request(
            endpoint=f"users/{username}", session=session
        )
    ...
    async def fetch_repository_languages(
        ...
        try:
            data = await self._make_request(
                endpoint=f"repos/{owner}/{repo}/languages", session=session
            )
            return data
        except Exception as e:
...
async def fetch_github_data(
    username: str, force_refresh: bool = False, session: Optional[AsyncSession] = None
) -> Dict[str, Any]:
    """
...
async def sync_github_profile(
    session: AsyncSession, username: str, force_refresh: bool = False
    ...
    try:
        ...
        logger.error(
            f"Unexpected error while syncing GitHub profile {safe_username}: {e}"
        )
        raise GitHubAPIError(f"Failed to sync GitHub profile: {e}")


def normalize_github_repository(
    repo_data: Dict[str, Any], languages_data: Optional[Dict[str, int]] = None
) -> Dict[str, Any]:
...
async def fetch_github_repository_responses(
    username: str, limit: int = 6
) -> List[Dict[str, Any]]:
...
async def sync_github_repositories(
    username: str, session: AsyncSession, force_refresh: bool = False
    ...
    if not force_refresh:
        ...
        if cached_repos:
            ...
            latest_sync = max(
                [r.last_synced_at for r in cached_repos if r.last_synced_at],
                default=None,
            )
            if latest_sync:
                if (
                    latest_sync.tzinfo is None
                    or latest_sync.tzinfo.utcoffset(latest_sync) is None
                ):
    ...
    if not profile:
        profile_data = await fetch_github_data(
            username=username, force_refresh=True, session=session
        )
