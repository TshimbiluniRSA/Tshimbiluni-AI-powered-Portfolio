...
class GitHubProfile(Base, TimestampMixin):
    ...
    username = Column(
        ...
        primary_key=True,
        index=True,
        doc="GitHub username (max 39 chars as per GitHub rules)",
    )
    ...
    last_fetched_at = Column(
        DateTime(timezone=True),
        server_default=func.now(),
        doc="When the data was last fetched from GitHub API",
    )
    ...
    def is_data_stale(self) -> bool:
        ...
        if (
            last_fetched.tzinfo is None
            or last_fetched.tzinfo.utcoffset(last_fetched) is None
        ):
...
class GitHubRepository(Base, TimestampMixin):
    ...
    github_id = Column(
        BigInteger, unique=True, index=True, nullable=False, doc="GitHub's repo ID"
    )
    owner_username = Column(
        String(39), ForeignKey("github_profiles.username"), index=True
    )
    ...
    languages_data = Column(
        JSON, doc="Language breakdown e.g. {'Python': 5000, 'JavaScript': 3000}"
    )
...
class GitHubStatsSnapshot(Base, TimestampMixin):
    """A render-ready, cached snapshot of official GitHub API data."""

    __tablename__ = "github_stats_snapshots"
    ...
    total_watchers = Column(Integer, nullable=False, default=0)
    total_open_issues = Column(Integer, nullable=False, default=0)
    total_repository_size_kb = Column(BigInteger, nullable=False, default=0)
    total_contributions = Column(Integer, nullable=False, default=0)
    commit_contributions = Column(Integer, nullable=False, default=0)
    pull_request_contributions = Column(Integer, nullable=False, default=0)
    issue_contributions = Column(Integer, nullable=False, default=0)
    pull_request_review_contributions = Column(Integer, nullable=False, default=0)
    contribution_period_start = Column(String(10))
    contribution_period_end = Column(String(10))
    language_stats_json = Column(JSON, nullable=False, default=list)
    recent_repositories_json = Column(JSON, nullable=False, default=list)
    synced_at = Column(
...
class CV(Base, TimestampMixin):
    ...
    id = Column(
        ...
        primary_key=True,
        autoincrement=True,
        doc="Unique identifier for the CV record",
    )
    ...
    mime_type = Column(
        String(100), default="application/pdf", doc="MIME type of the file"
    )
    ...
    experience = Column(
        JSON, doc="Work experience e.g. [{title, company, duration, description}]"
    )
    ...
    parsing_status = Column(
        String(20), default="pending", doc="pending, success, failed"
    )
    parsing_error = Column(Text, doc="Error message if parsing failed")
    ai_model_used = Column(
        String(50), doc="AI model used for parsing, e.g. 'gpt-5-mini'"
    )
    ...
    __table_args__ = (Index("idx_cv_user_active", "user_id", "is_active"),)
...
class CVMetadata(Base, TimestampMixin):
    ...
    is_active = Column(
        Boolean, default=True, doc="Whether this CV version is currently active"
    )
    download_count = Column(
        Integer, default=0, doc="Number of times this CV has been downloaded"
    )
...
class ChatHistory(Base, TimestampMixin):
    ...
    id = Column(
        ...
        primary_key=True,
        autoincrement=True,
        doc="Unique identifier for the chat message",
    )
    session_id = Column(
        String(255), nullable=False, index=True, doc="Chat session identifier"
    )
    message_type = Column(
        String(20), nullable=False, doc="Type of message: 'user' or 'assistant'"
    )
    content = Column(Text, nullable=False, doc="Message content")
    msg_metadata = Column(
        JSON, doc="Additional metadata like tokens used, model info, etc."
    )
    response_time_ms = Column(
        Integer, doc="Response time in milliseconds for assistant messages"
    )
...
class APIUsageLog(Base, TimestampMixin):
    ...
    id = Column(
        ...
        primary_key=True,
        autoincrement=True,
        doc="Unique identifier for the API call log",
    )
    api_provider = Column(
        String(50), nullable=False, doc="API provider (github or openai)"
    )
